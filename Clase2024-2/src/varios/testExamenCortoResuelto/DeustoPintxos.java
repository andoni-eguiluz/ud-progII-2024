...
//		System.out.println("El dia de mayor recaudacion ha sido el " + diaMayorRecaudacion(bares));
		...
		for (Producto p : productos) {
			if (p instanceof Concursable) {
				Concursable c = (Concursable) p;
				if (c.enConcurso()) {
		...
		// TAREA 2
		System.out.println();
		TreeMap<Producto,ArrayList<String>> mapa = new TreeMap<>();
		for (Bar bar : bares) {
			for (Pedido pedido : bar.getPedidos()) {
				for (Producto prod : pedido.getProductos().keySet()) {
					if (!mapa.containsKey( prod )) {
						mapa.put( prod, new ArrayList<>() );
						mapa.get( prod ).add( bar.getNombre() );
					} else {
						mapa.get( prod ).add( bar.getNombre() );
					}
				}
		...
		try {
			ObjectOutputStream oos = new ObjectOutputStream( new FileOutputStream( "mapa.dat" ) );
			oos.writeObject( mapa );
			oos.close();
		} catch (Exception e) {
			System.out.println( "Error en salida fichero" );
			e.printStackTrace();
		}
		for (Producto p : mapa.keySet()) {
			System.out.println( p + " --> en " + mapa.get(p).size() + " pedidos = " + mapa.get(p) );
		}
	}
