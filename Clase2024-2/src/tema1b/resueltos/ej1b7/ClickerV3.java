package tema1b.resueltos.ej1b7;
...
public class ClickerV3 {
	...
	private static GrupoFiguras circulos;
	// Ladrillos
	private static GrupoFiguras rectangulos;
	...
	private static void juego() {
		...
		while (!ventana.estaCerrada()) {
			...
			if (System.currentTimeMillis() - tiempoUltimoCirculo > ESPERA_ENTRE_CIRCULOS) {
				circulos.add( new Circulo( ventana ) );
			...
			for (int i=rectangulos.size()-1; i>=0; i--) {  // ¿por qué al revés? (Observa lo que ocurriría si no fuera así)
				Rectangulo rect = (Rectangulo) rectangulos.get(i);
	...
	private static Circulo hayClickEnAlgunCirculoOLadrillo( GrupoFiguras circulos, Point hayClick ) {
		if (hayClick!=null) {
			// Chequeo de ladrillo:
			Figura ladrilloConClick = rectangulos.encuentraFiguraEnPunto(hayClick);
			if (ladrilloConClick!=null) {
				...
				// Chequeo de círculo
				Figura circuloConClick = circulos.encuentraFiguraEnPunto(hayClick);
