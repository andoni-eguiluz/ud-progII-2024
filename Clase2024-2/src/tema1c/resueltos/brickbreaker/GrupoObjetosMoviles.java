	...
	public void choqueMultipleConOtrosObjetos( VentanaGrafica v, int milis, GrupoObjetosMoviles gObjetos ) {
		for (int i=0; i<lOAs.size(); i++) {
			...
			if (!(objetoA instanceof Chocable) || (!((Chocable)objetoA).esChocable() && !((Chocable)objetoA).esTocable())) {
				continue;  // No se procesa el objeto si no es chocable ni tocable
			}
			ArrayList<ObjetoMovil> lChoques = new ArrayList<>();
			for (int j=0; j<gObjetos.size(); j++) {
				ObjetoMovil objetoA2 = gObjetos.get(j);
				if (objetoA2 instanceof Chocable && (((Chocable)objetoA2).esChocable() || ((Chocable)objetoA2).esTocable())) {
					if (((Chocable)objetoA2).chocaCon( objetoA )) {
						if (((Chocable)objetoA).esChocable()) {  // Si es chocable se calcula choque-rebote, si no simplemente se le toca (método chocaCon)
			...
			for (ObjetoMovil oa : lChoques) {
				if (oa instanceof ObjetoVivo) {
					((ObjetoVivo) oa).decVida();
				}
			}
		}
	}
