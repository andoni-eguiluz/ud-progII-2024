package tema1c.resueltos.acertijo;
...
public class Acertijo {
	...
	private boolean dragObjeto( Draggable obj, Point pInicial, Point pActual ) {
		...
		else if (obj instanceof Circulo && barca.getObjetosContenidos().contains( (ObjetoAcertijo)obj )) {   // El personaje está originalmente en la barca
		...
		else if (obj instanceof Circulo) { // Está en una orilla
				...
				if (orilla != orillaSegura) {
					...
					for (ObjetoAcertijo o1 : orilla.getObjetosContenidos()) {
						for (ObjetoAcertijo o2 : orilla.getObjetosContenidos()) {
							if (o1!=o2) {
								if (o1 instanceof Comible) {
									if (((Comible)o1).puedeSerComidoPor( o2 )) {
	...
	private void dibujaTodo( Draggable objetoFinal ) {
		...
		for (ObjetoAcertijo o : listaObjetos) {
			...
			if (o instanceof Rectangulo) {
				...
				System.out.println( o + " - área " + (r.getAnchura()*r.getAltura()));
			}
		}
		if (objetoFinal!=null) {
