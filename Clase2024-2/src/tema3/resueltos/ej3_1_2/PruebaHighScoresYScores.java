...
public class PruebaHighScoresYScores {
	...
	private static void pideYAnyadeScore() {
		...
		misScores.addScore( usuario, puntuacionNumerica );
		misHighScores.addHighScore( usuario , puntuacionNumerica );
		...
		while (ficEntrada.hasNextLine()) {
			...
			if (valores.length>1) {
				for (int i=1;i<valores.length;i++) {
					misScores.addScore( valores[0], Integer.parseInt( valores[i] ) );
					misHighScores.addHighScore( valores[0], Integer.parseInt( valores[i] ) );
				}
			}
		}
		ficEntrada.close();
